...
public class WDBenchTest {
    ...
    static BlazegraphBackend wdbenchBlazegraph = new BlazegraphBackend("/GDD/Thi/run_blazegraph_wdbench/wdbench-blaze/blazegraph.jnl");
...
//                } }""";
    ...
    @Disabled
    @Test
    public void filter_out_queries_with_cartesian_products () {
        List<Pair<String, String>> queries = Watdiv10M.getQueries("/GDD/Thi/wdbench_multiple_bgps_count_all", List.of());
        List<Pair<String, String>> filtered = new ArrayList<>();
        for (Pair<String, String> nameAndQuery : queries) {
            Op original = Algebra.compile(QueryFactory.create(nameAndQuery.getRight()));
            CardinalityJoinOrdering cjo = new CardinalityJoinOrdering(wdbenchBlazegraph);
        ...
        log.info("Kept {} queries out of {}.", filtered.size(), queries.size());
        for (Pair<String, String> nameAndQuery : filtered) {
            System.out.println(nameAndQuery.getLeft());
        }
    }
}
