...
def plot_data(input_data_files, note_texts_1,note_texts_2, output_plot_file):
    ...
    fig, axes = plt.subplots(nrows=1, ncols=len(input_data_files), figsize=(6.7*3, 2.3*3 ), gridspec_kw={'wspace': 0.1})
    ...
    for i, input_data_file in enumerate(input_data_files):
        ...
        if i==0:
            ...
            ax.text(0.65, 0.5, note_texts_2[i], ha='right', va='bottom', transform=ax.transAxes, fontsize=fontsize)
            ax.text(0.1, 0.95, "OBJECT", ha='center', va='bottom', transform=ax.transAxes, fontsize=fontsize-2, weight='bold')
            ...
            ax.text(1, 0.1, note_texts_1[i], ha='right', va='bottom', transform=ax.transAxes, fontsize=fontsize)
            ax.set_xlim(0, 0.01)
...
input_data_files = ['/GDD/Thi/count-distinct-sampling/watdiv/final_csv/SPO_CDo.csv', '/GDD/Thi/count-distinct-sampling/largerdf_dbpedia/final_csv/SPO_CDo.csv', '/GDD/Thi/count-distinct-sampling/wdbench/final_csv/SPO_CDo.csv']
note_texts_1 = ['1.1M samples', '4.5M samples', '13M samples']
note_texts_2 = ['exact value = 1,005,832', 'exact value = 13,619,093', 'exact value = 304,967,140']
output_plot_file = '/GDD/Thi/count-distinct-sampling/CDo_merged.png'
